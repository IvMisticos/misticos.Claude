...
The modern-web-guidance skill holds guides on current browser APIs: dialogs, popovers, view transitions, forms, passkeys, performance. Training data leans on legacy patterns, so search it before writing web code and run it with `bunx`. Use Baseline widely available features without fallback. Use newly available features behind feature detection, with a fallback and never a polyfill.
